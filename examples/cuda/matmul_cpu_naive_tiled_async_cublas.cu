...
__global__ void matmulTiled(const float *a, const float *b, float *c, int n) {
  ...
  __shared__ float tileA[TILE][TILE + 1];  // +1 padding
  __shared__ float tileB[TILE][TILE + 1];  // +1 padding
...
int main(int argc, char **argv) {
  ...
  if (n <= 0 || iterations <= 0 || tileSize <= 0 || asyncStreams <= 0) {
    std::cerr << "Usage: ./matmul_cpu_naive_tiled_async_cublas [matrix_size] "
              << "[iterations] [tile_size] [async_streams]\n"
              << "Example: ./matmul_cpu_naive_tiled_async_cublas 2048 30 16 2"
              << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::cout << "cublas_async_h2d_gemm_d2h," << cublasAsyncMs << ","
            << cublasAsyncGflops << "\n";
  std::cout << "speedup_naive_vs_cpu," << (cpuMs / naiveMs) << "\n";
  std::cout << "speedup_tiled_vs_cpu," << (cpuMs / tiledMs) << "\n";
  std::cout << "speedup_cublas_async_vs_cpu," << (cpuMs / cublasAsyncMs) << "\n";
